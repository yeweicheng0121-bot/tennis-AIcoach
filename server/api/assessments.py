...
async def list_assessments(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    return [{"assessment_id": str(a.id), "overall_ntrp": a.overall_ntrp, "ntrp_confidence": a.ntrp_confidence, "strengths": a.strengths, "weaknesses": a.weaknesses, "created_at": a.created_at.isoformat() if a.created_at else None} for a in result.scalars().all()]
...
async def get_assessment(assessment_id: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    return {"assessment_id": str(a.id), "overall_ntrp": a.overall_ntrp, "ntrp_confidence": a.ntrp_confidence, "technique_breakdown": a.technique_breakdown, "fitness_breakdown": a.fitness_breakdown, "strengths": a.strengths, "weaknesses": a.weaknesses, "key_frames": a.key_frames, "report_markdown": a.report_markdown, "created_at": a.created_at.isoformat() if a.created_at else None}
